...
const CourseBuilderForm = () => {
  ...
  return (
    <div className="bg-richblack-800 rounded-md p-5 sm:p-8 border border-richblack-700 text-richblack-5">
      <p className="text-xl sm:text-2xl font-medium mb-5 sm:mb-9">
        Course Builder
      </p>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="flex flex-col gap-2">
          <label className="text-sm text-richblack-5" htmlFor="sectionName">
            Section<sup className="text-pink-400 text-base">*</sup>
          </label>
          <input
            name="sectionName"
            id="sectionName"
          ...
          />
          {errors.sectionName && (
            <span className="text-yellow-100">Please enter section title</span>
          )}
        </div>
        ...
        <div className="mt-4 sm:mt-5 flex items-center gap-3">
          <IconBtn
            ...
            outline={true}
            customClasses={
              "border-1 border border-yellow-50 flex gap-2 text-yellow-100 text-base font-semibold"
            }
          >
            <IoAddCircleOutline
              size={20}
          ...
          {editSectionID && (
            <button
              onClick={cancelEdit}
              type="button"
              className="text-sm text-richblack-200 hover:text-richblack-100"
            >
              Cancel edit
            </button>
          )}
      ...
      <div className="flex justify-end gap-x-2 sm:gap-x-3">
        ...
        px-4 sm:px-5 py-2 text-center rounded-md transition-all duration-200 hover:scale-95 flex items-center font-semibold"
          onClick={goToNext}
        >
          Next
          <MdNavigateNext />
