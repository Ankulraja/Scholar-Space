...
const CourseDetails = () => {
  ...
  const getCourseFullDetails = async () => {
    try {
      ...
      setActiveStatus(result?.courseContent?.[0]?._id);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    courseData.courseContent.forEach((sec) => {
      ...
      lectures += subSections.length;
  
      subSections.forEach((subSec) => {
        duration += Number(subSec?.timeDuration) || 0;
      });
    });
        ...
        <div className="flex flex-col gap-x-3 justify-between">
          <div className="flex gap-x-3 justify-between">
            <div className="flex gap-2">
              ...
              <span>{totalDuration?.toFixed(1)}s total length</span>
            </div>
